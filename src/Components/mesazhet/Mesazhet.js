...
function Mesazhet() {
    ...
    function handleEdito(id) {
        axios.put(`https://localhost:7156/api/ContactForm/konfirmoMesazhin?id=${id}`, {}, authentikimi)
            .then(x => {
    ...
    async function handleFshij(id) {
        try {
            await axios.delete(`https://localhost:7156/api/ContactForm/fshiMesazhin?id=${id}`, authentikimi);
            setTipiMesazhit("success");
    ...
    useEffect(() => {
        const shfaqKompanit = async () => {
            try {
                setLoading(true);
                const mesazhet = await axios.get("https://localhost:7156/api/ContactForm/shfaqMesazhet", authentikimi);

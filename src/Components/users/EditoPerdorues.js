...
function EditoPerdorues(props) {
    ...
    useEffect(() => {
        const vendosPerdoruesin = async () => {
            try {
                const user = await axios.get(`https://localhost:7156/api/Perdoruesi/shfaqSipasID?idUserAspNet=${props.id}`, authentikimi);
    ...
    useEffect(() => {
        const vendosRolet = async () => {
            try {
                const rolet = await axios.get(`https://localhost:7156/api/Authenticate/shfaqRolet`, authentikimi);
    ...
    async function handleSubmit() {
        try {
            for (const y of roletUseri) {
                if (y !== 'User') {
                    await axios.delete(`https://localhost:7156/api/Authenticate/FshijRolinUserit?userID=${props.id}&roli=${y}`, authentikimi);
                }
            }

            for (const y of roletSelektim) {
                if (y !== 'User') {
                    await axios.post(`https://localhost:7156/api/Authenticate/ShtoRolinPerdoruesit?userID=${props.id}&roli=${y}`, {}, authentikimi);
                }
            }

...
function EditoKompanin(props) {
        ...
        const vendosKompanite = async () => {
            try {
                const kompanit = await axios.get(
                    `https://localhost:7156/api/Kompania/shfaqKompanit`, authentikimi
                );
    ...
    useEffect(() => {
        const shfaqKompanit = async () => {
            try {
                const kompania = await axios.get(`https://localhost:7156/api/Kompania/shfaqKompanin?id=${props.id}`, authentikimi);
    ...
    async function handleSubmit() {
        if (foto) {
            ...
            try {
                await axios.post(`https://localhost:7156/api/VendosFotot/EditoKompanin?fotoVjeterKompanis=${kompania.logo}`, formData, authentikimi)
                    .then(async (response) => {
                        await axios.put(`https://localhost:7156/api/Kompania/perditesoKompanin?id=${kompania.kompaniaId}`,
                            {

...
function ZbritjetEProduktit(props) {
    ...
    useEffect(() => {
        const shfaqZbritjet = async () => {
            try {
                setLoading(true);
                const zbritja = await axios.get("https://localhost:7156/api/ZbritjaQmimitProduktit/shfaqZbritjet", authentikimi);
    ...
    const fshijZbritjen = (id) => {
        axios.delete(`https://localhost:7156/api/ZbritjaQmimitProduktit/fshijZbritjenProduktit?id=${id}`, authentikimi)

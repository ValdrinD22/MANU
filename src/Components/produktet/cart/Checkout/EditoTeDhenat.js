...
const EditoTeDhenat = (props) => {
  ...
  useEffect(() => {
    const vendosPerdoruesin = async () => {
      try {
        const perdoruesi = await axios.get(`https://localhost:5289/api/Perdoruesi/shfaqSipasID?idUserAspNet=${getID}`, authentikimi);
  ...
  const handlePerditesoTeDhenat = () => {
    if (
      ...
      axios.put(`https://localhost:5289/api/Perdoruesi/perditesoPerdorues/${perdoruesi.userId}`, perdoruesi, authentikimi);

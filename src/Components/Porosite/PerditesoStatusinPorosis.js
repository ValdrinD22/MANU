...
function PerditesoStatusinPorosis(props) {
    ...
    useEffect(() => {
        const vendosPorosine = async () => {
            try {
                const porosia = await axios.get(`https://localhost:7156/api/Porosia/shfaqPorosineNgaID?nrFatures=${props.id}`, authentikimi);
    ...
    function handleSubmit() {
        ...
        axios.put(`https://localhost:7156/api/Porosia/perditesoStatusinPorosis?idPorosia=${props.id}&statusi=${statusiPorosis}`, {}, authentikimi)
            .then(x => {

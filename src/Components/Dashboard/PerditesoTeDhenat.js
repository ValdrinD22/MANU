    ...
    async function EditoTeDhenat(e) {
        ...
        try {
            if (
                ...
                const kontrolloEmail = await axios.get(`https://localhost:7156/api/Perdoruesi/KontrolloEmail?email=${formValue.email}`, authentikimi)

                if (kontrolloEmail.data === false || teDhenat.perdoruesi.email === formValue.email) {
                    await axios.post(`https://localhost:7156/api/Perdoruesi/NdryshoEmail?emailIVjeter=${teDhenat.perdoruesi.email}&emailIRI=${formValue.email}`, {}, authentikimi)
                        .then(async () => {
                            await axios.put(`https://localhost:7156/api/Perdoruesi/perditesoPerdorues/${teDhenat.perdoruesi.userId}`, {
                                "emri": formValue.emri,
    ...
    async function EditoFjalekalimin(e) {
        ...
        try {
            if (
                ...
                const kontrolloFjalekalimin = await axios.get(`https://localhost:7156/api/Perdoruesi/KontrolloFjalekalimin?AspNetID=${getID}&fjalekalimi=${fjalekalimiAktual}`, authentikimi)

                if (kontrolloFjalekalimin.data === true) {
                    if (passREGEX.test(fjalekalimiIRi)) {
                        await axios.post(`https://localhost:7156/api/Perdoruesi/NdryshoFjalekalimin?AspNetID=${getID}&fjalekalimiAktual=${fjalekalimiAktual}&fjalekalimiIRi=${fjalekalimiIRi}`,
                            {}, authentikimi);
